...
export default class TransactionScreen extends Component {
  ...
  checkBook=async bookId=>{
    ...
    .get()
    var transactionType = ""
    if (refBook.docs.length == 0) {
      transactionType = false
    }
    else {
      refBook.docs.map(doc=>{
  ...
  checkStudent=async studentId=>{
    const refStudent = await db
    .collection("students")
    .where("id", "==", studentId)
    .get()
    var checkStudentV = ""
    if (refStudent.docs.length == 0) {
      this.setState({
        bookId: "",
        studentId: ""
      })
      checkStudentV = false
      Alert.alert("O ID do Aluno não existe na escola.")
    }
    else {
      refStudent.docs.map(doc=>{
